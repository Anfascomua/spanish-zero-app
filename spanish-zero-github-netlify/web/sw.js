const CACHE = 'spanish-zero-web-v7-card-audio-sync';
...
const LOOP_SCRIPT = String.raw`(() => {
  ...
  async function play(id){while(running&&id===runId&&deck.length){if(typeof page!=='undefined'&&page!=='drill'){stop();break}const index=step%deck.length,w=deck[index],femaleFirst=step%2===0;syncCard(w,index);if(!await pause(90,id))break;await sayPart(w.es,'es-ES',femaleFirst?'female':'male',.78);if(!running||id!==runId)break;if(typeof page!=='undefined'&&page!=='drill'){stop();break}if(!await pause(260,id))break;await sayPart(w.ru,'ru-RU',femaleFirst?'male':'female',.88);if(!running||id!==runId)break;step++;if(!await pause(600,id))break}}
  function toggle(){if(running){stop();return}const p=progress(),selected=Array.isArray(p.dictSelected)?p.dictSelected:[];deck=entries().filter(x=>selected.includes(x.id));if(!deck.length){alert('Сначала выберите слова флажком «Учить».');return}if(!('speechSynthesis'in window)){alert('Этот браузер не поддерживает системную озвучку.');return}speechSynthesis.cancel();running=true;step=0;const id=++runId;syncCard(deck[0],0);play(id)}
