const CACHE = 'spanish-zero-web-v11-pause-3s-alt-voices';
...
const LOOP_SCRIPT = String.raw`(() => {
  ...
  function makePause(gender){const u=makeUtterance('pausa pausa pausa','es-ES',gender,.5);u.volume=0;return u}
  function refresh(){const b=document.getElementById('selected-loop-btn'),s=document.getElementById('selected-loop-status'),n=count();if(b){b.disabled=!n&&!running;b.style.opacity=(n||running)?'1':'.45';const label=running?'■ Остановить прослушку':('🔁 Прослушка по кругу ('+n+')');if(b.textContent!==label)b.textContent=label}if(s){const label=running?('Идёт по кругу: '+deck.length+' слов. Между словами пауза 3 секунды. Голоса чередуются мужской/женский.'):'1-е слово: женщина по-испански → мужчина по-русски. 2-е: мужчина по-испански → женщина по-русски. Между словами пауза 3 секунды.';if(s.textContent!==label)s.textContent=label}const stopBtn=document.getElementById('loop-stop-card-btn');if(stopBtn)stopBtn.textContent='■ Остановить прослушку'}
  ...
  function queueBackground(id){if(!running||id!==runId||!deck.length)return;queuedWords=0;for(let turn=0;turn<PREQUEUE_WORDS;turn++){const index=turn%deck.length,w=deck[index],femaleFirst=turn%2===0;const esGender=femaleFirst?'female':'male',ruGender=femaleFirst?'male':'female';const es=makeUtterance(w.es,'es-ES',esGender,.70);const ru=makeUtterance(w.ru,'ru-RU',ruGender,.79);const gap=makePause(esGender);es.onstart=()=>{if(running&&id===runId){queuedWords=turn+1;syncCard(w,index)}};es.onerror=()=>{};ru.onerror=()=>{};gap.onerror=()=>{};speechSynthesis.speak(es);speechSynthesis.speak(ru);speechSynthesis.speak(gap)}}
